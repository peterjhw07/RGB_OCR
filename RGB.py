...
#  Insert parameters
directory = r'C:\Users\Peter\Documents\Postdoctorate\Work\CAKE\Photos\M.17 RXN Timelapse - DPA CAKE'  # input path
...
exportpath = r'C:\Users\Peter\Documents\Postdoctorate\Work\CAKE\Blank.xlsx'  # enter Excel path for export
...
exportdf = pd.DataFrame({"File": export_files, "Date time": total_time_data,
                         "Pri_R": total[:, 0], "Sec_R": total[:, 3], "Ter_R": total[:, 6],
                         "Pri_G": total[:, 1], "Sec_G": total[:, 4], "Ter_G": total[:, 7],
                         "Pri_B": total[:, 2], "Sec_B": total[:, 5], "Ter_B": total[:, 8]})  # include desired regions/colours
